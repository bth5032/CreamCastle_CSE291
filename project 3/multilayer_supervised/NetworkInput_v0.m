classdef NetworkInput_v0 < matlab.mixin.Copyable
    ...
    methods
        ...
        function pca(obj, num_orientations, num_principal_components)
            % Convert the convolved_features property into a matrix, split
            % it for each orientation.
            full_data_mat = cell2mat(obj.convolved_features)
            num_examples  = length(obj.data)
            full_data_mat = reshape(full_data_mat, [], num_examples)
            
            
            [U, S, V]     = svds(data_mat, num_principal_components)
        end
